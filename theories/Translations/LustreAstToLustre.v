...
Record common_temp : Type := {
  ...
  Hnin:
    Forall2 (fun b '(v, tv) =>
        binder_name b = v /\
        StringMap.find v smap = Some (binder_id b, Result.DeclInput) /\
        binder_ty b = tv)
      n_in orig.(Source.n_in);
  Hnout:
    Forall2 (fun b '(v, tv) =>
        binder_name b = v /\
        StringMap.find v smap = Some (binder_id b, Result.DeclOutput) /\
        binder_ty b = tv)
      n_out orig.(Source.n_out);
  Hnloc:
    Forall2 (fun b '(v, tv) =>
        binder_name b = v /\
        StringMap.find v smap = Some (binder_id b, Result.DeclLocal) /\
        binder_ty b = tv)
      n_locals orig.(Source.n_locals);
  
  seed: seed_ty;
  Hseed: freshness seed nvars;
  
  env: Dict.t (string * type);
  Henv: forall b, Dict.maps_to (binder_id b) (binder_name b, binder_ty b) env <-> In b (n_in ++ n_out ++ n_locals);
}.
...
Proof.
  ...
  - destruct (StringMap.find n (smap temp)) as [ [ i dl ] | ] eqn:eqi.
    ...
    destruct (Dict.find i (env temp)) as [ [ n' ty] | ] eqn:eqty.
    2: right; exact [(l, Result.InternalError ("Variable " ++ n ++ " has an ID but no type"))].
    left.
    exists ty, (Target.EVar l {| binder_name := n'; binder_id := i; binder_ty := ty |}).
    intros ? [<-|[]]; cbn.
    apply temp.(Henv).
    exact eqty.
...
Proof.
  ...
  refine (Result.bind _
    (fun res : { rem & { body | Permutation (rem ++ map Target.equation_dest body) (n_out temp ++ n_locals temp) /\
      ...
      | existT _ (b :: _) _ => Result.Err [(nloc, Result.MissingAssignment (binder_name b) (binder_id b) (binder_ty b))] end)).
  ...
  - refine (Result.bind (Result.combine (check_exp temp e) IH) _); clear IH.
    ...
    pose (rev_lhs := @nil binder); change rem with (rev_lhs ++ rem) in IH1.
    generalize dependent rev_lhs; induction rem as [ | [ n' i' ty' ] rem IH ]; intros rev_lhs Hperm.
    1: refine (Result.Err [(l, _)]); destruct temp as [? ? ? nin ? ? ?]; clear - n i nin.
    1: induction nin as [|[hdn hdi hdt] tl IH].
    1:  exact (Result.UndeclaredVariable n).
    1: destruct (PeanoNat.Nat.eq_dec i hdi) as [_|_].
    1:  exact (Result.AssignToInput n i hdt).
    1: exact IH.
    destruct (PeanoNat.Nat.eq_dec i i') as [ <- | nnei ].
    2: refine (IH ({| binder_name := n'; binder_id := i'; binder_ty := ty' |} :: rev_lhs) (Permutation_trans (Permutation_app_tail _ _) Hperm)).
    2: rewrite (app_assoc _ [_] _ : _ ++ {| binder_name := n'; binder_id := i'; binder_ty := ty' |} :: _ = _).
    ...
    refine (Result.Ok (existT _ (rev_append rev_lhs rem) (exist _ (((n', i), existT Target.exp ty e') :: eqs) _))).
...
Definition vars_unique (l: Result.location) (temp: common_temp) :
  Result.t type (NoDup ((map binder_id (n_in temp ++ n_out temp ++ n_locals temp)))).
Proof using.
  ...
  2:{
    ...
    destruct (In_dec PeanoNat.Nat.eq_dec (binder_id hd) (map binder_id tl)) as [ _ | nin ].
    1: exact (Result.Err [(l, Result.MultipleDeclaration (binder_name hd) (binder_id hd) Result.DeclInput Result.DeclInput)]).
    destruct (In_dec PeanoNat.Nat.eq_dec (binder_id hd) (map binder_id (n_out temp))) as [ _ | nout ].
    1: exact (Result.Err [(l, Result.MultipleDeclaration (binder_name hd) (binder_id hd) Result.DeclInput Result.DeclOutput)]).
    destruct (In_dec PeanoNat.Nat.eq_dec (binder_id hd) (map binder_id (n_locals temp))) as [ _ | nloc ].
    1: exact (Result.Err [(l, Result.MultipleDeclaration (binder_name hd) (binder_id hd) Result.DeclInput Result.DeclLocal)]).
    refine (Result.Ok _).
  ...
  2:{
    refine (Result.bind (Result.combine_prop _ IH) (fun '(conj H1 H2) => Result.Ok (NoDup_cons _ H1 H2))); clear IH.
    destruct (In_dec PeanoNat.Nat.eq_dec (binder_id hd) (map binder_id tl)) as [ _ | nin ].
    1: exact (Result.Err [(l, Result.MultipleDeclaration (binder_name hd) (binder_id hd) Result.DeclOutput Result.DeclOutput)]).
    destruct (In_dec PeanoNat.Nat.eq_dec (binder_id hd) (map binder_id (n_locals temp))) as [ _ | nloc ].
    1: exact (Result.Err [(l, Result.MultipleDeclaration (binder_name hd) (binder_id hd) Result.DeclOutput Result.DeclLocal)]).
    refine (Result.Ok _).
    rewrite !map_app, !in_app_iff; tauto.
  }
  ...
  2:{
    refine (Result.bind (Result.combine_prop _ IH) (fun '(conj H1 H2) => Result.Ok (NoDup_cons _ H1 H2))); clear IH.
    destruct (In_dec PeanoNat.Nat.eq_dec (binder_id hd) (map binder_id tl)) as [ _ | nin ].
    1: exact (Result.Err [(l, Result.MultipleDeclaration (binder_name hd) (binder_id hd) Result.DeclLocal Result.DeclLocal)]).
    refine (Result.Ok _).
    rewrite !map_app, !in_app_iff; tauto.
  }
...
Definition build_map (entry: Source.node) :
  { sm & { seed & Result.t type { n_in & { n_out & { n_locals |
    Forall (fun v => forall b', binder_id b' = binder_id v -> In b' n_in -> b' = v) n_in /\
    Forall (fun v => forall b', binder_id b' = binder_id v -> In b' n_out -> b' = v) n_out /\
    Forall (fun v => forall b', binder_id b' = binder_id v -> In b' n_locals -> b' = v) n_locals /\
    Forall (fun v => ~ In (binder_id v) (map binder_id n_out)) n_in /\
    Forall (fun v => ~ In (binder_id v) (map binder_id n_locals)) n_in /\
    Forall (fun v => ~ In (binder_id v) (map binder_id n_locals)) n_out /\
    Forall2 (fun b '(v, tv) =>
        binder_name b = v /\
        StringMap.find v sm = Some (binder_id b, Result.DeclInput) /\
        binder_ty b = tv)
      n_in entry.(Source.n_in) /\
    Forall2 (fun b '(v, tv) =>
        binder_name b = v /\
        StringMap.find v sm = Some (binder_id b, Result.DeclOutput) /\
        binder_ty b = tv)
      n_out entry.(Source.n_out) /\
    Forall2 (fun b '(v, tv) =>
        binder_name b = v /\
        StringMap.find v sm = Some (binder_id b, Result.DeclLocal) /\
        binder_ty b = tv)
      n_locals entry.(Source.n_locals) /\
    freshness seed (n_in ++ n_out ++ n_locals) } } } } }.
Proof using.
  ...
  2:{
    ...
    destruct (next_ident seed n ty) as [seed2 newb] eqn:eqseed1.
    exists (StringMap.add n (seed, Result.DeclInput) sm), seed2;
      refine (Result.bind IH _); clear IH;
      intros (IHnin & IHout & IHloc & IHii & IHoo & IHll & IHio & IHil & IHol & IH1 & IH2 & IH3 & IH4);
      left; exists (newb :: IHnin), IHout, IHloc.
    repeat match goal with |- _ /\ _ => split end; try assumption.
    + constructor; [intros [bn' bi' bty'] [=->] [->|H]; [exact eq_refl|exfalso; refine (IH4 [] _ _ _)]|].
      1: injection eqseed1 as <- <-; rewrite map_app; exact (in_or_app _ _ _ (or_introl (in_map _ _ _ H))).
      assert (Hseed : ~ In (binder_id newb) (map binder_id IHnin))
        by (injection eqseed1 as <- <-; unfold freshness in IH4; rewrite map_app in IH4; exact (fun f => IH4 [] _ _ (in_or_app _ _ _ (or_introl f)))).
      clear - IHii Hseed.
      refine (proj2 (Forall_forall _ _) _).
      intros [vn v tv] Hv [bn' bi' bt'] [=->] [[=f]|H]; [contradict Hseed; subst; exact (in_map _ _ _ Hv)|].
      refine (proj1 (Forall_forall _ _) IHii _ Hv _ _ H); exact eq_refl.
    + constructor; [|assumption]. refine (fun f => IH4 [] _ _ _).
      injection eqseed1 as <- <-.
    ...
    + injection eqseed1 as <- <-.
      constructor; [split; [exact eq_refl|split; [|exact eq_refl]]|].
      1: exact (StringMap.find_1 (StringMap.add_1 _ _ eq_refl)).
      refine (Forall2_impl _ _ IH1); clear - eqnsm.
      intros [tn x tx] [v tv] [H1 [H2 H3]]; split; [exact H1|split; [cbn in H2 |- *; clear tn tx tv H1 H3|exact H3]].
      refine (StringMap.find_1 (StringMap.add_2 _ _ (StringMap.find_2 H2))).
      intros <-; rewrite eqnsm in H2; discriminate H2.
    + refine (Forall2_impl _ _ IH2); clear - eqnsm.
      intros b [? ?] (<- & H & <-); split; [exact eq_refl|split; [|exact eq_refl]].
      apply StringMap.find_1.
      refine (StringMap.add_2 _ _ (StringMap.find_2 H)).
      intros ->.
      rewrite H in eqnsm; discriminate eqnsm.
    + refine (Forall2_impl _ _ IH3); clear - eqnsm.
      intros b [? ?] (<- & H & <-); split; [exact eq_refl|split; [|exact eq_refl]].
      apply StringMap.find_1.
      refine (StringMap.add_2 _ _ (StringMap.find_2 H)).
      intros ->.
      rewrite H in eqnsm; discriminate eqnsm.
    + intros k vn vt [H|H]; injection eqseed1 as <- <-; [|exact (IH4 ((vn, vt) :: k) vn vt H)].
      exact (ident_diff seed ((""%string, TInt) :: k) ltac:(discriminate) H).
  }
  refine (match _ : { sm & { seed & Result.t _ { n_out & { n_locals |
                      _ /\ _ /\ _ /\ Forall2 _ _ _ /\ Forall2 _ _ _ /\ freshness _ (n_out ++ _)}}}} with
    | existT _ sm (existT _ seed (Result.Ok (existT _ n_out (exist _ n_locals
         (conj Hoo (conj Hll (conj Hol H0))))))) =>
        existT _ sm (existT _ seed (Result.Ok (
          existT _ [] (existT _ n_out (exist _ n_locals
            (conj (Forall_nil _) (conj Hoo (conj Hll (conj (Forall_nil _) (conj (Forall_nil _) (conj Hol (conj (Forall2_nil _) H0))))))))))))
    | existT _ sm (existT _ seed (Result.Err e)) => existT _ sm (existT _ seed (Result.Err e)) end).
  induction out as [ | [n ty] vars (sm & seed & IH) ].
  2:{
    destruct (StringMap.find n sm) as [ [ i dl ] | ] eqn:eqnsm.
    1: exists sm, seed;
       exact (Result.Err ((l, Result.MultipleDeclaration n i Result.DeclOutput dl) :: match IH with Result.Ok _ => [] | Result.Err es => es end)).
    destruct (next_ident seed n ty) as [seed2 newb] eqn:eqseed1.
    exists (StringMap.add n (seed, Result.DeclOutput) sm), seed2;
      refine (Result.bind IH _); clear IH;
      intros (IHout & IHloc & IHoo & IHll & IHol & IH1 & IH2 & IH3);
      left; exists (newb :: IHout), IHloc.
    repeat match goal with |- _ /\ _ => split end; try assumption.
    + constructor; [intros [bn' bi' bty'] [=->] [[=<-]|H]; [exact eq_refl|exfalso; refine (IH3 [] _ _ _)]|].
      1: injection eqseed1 as <- <-; rewrite map_app; exact (in_or_app _ _ _ (or_introl (in_map _ _ _ H))).
      assert (Hseed : ~ In (binder_id newb) (map binder_id IHout))
        by (injection eqseed1 as <- <-; unfold freshness in IH3; rewrite map_app in IH3; exact (fun f => IH3 [] _ _ (in_or_app _ _ _ (or_introl f)))).
      clear - IHoo Hseed.
      refine (proj2 (Forall_forall _ _) _).
      intros [vn v tv] Hv [bn' bi' bt'] [=->] [[=f]|H]; [contradict Hseed; subst; exact (in_map _ _ _ Hv)|].
      refine (proj1 (Forall_forall _ _) IHoo _ Hv _ _ H); exact eq_refl.
    + constructor; [|assumption]. refine (fun f => IH3 [] _ _ _).
      injection eqseed1 as <- <-.
      rewrite map_app; exact (in_or_app _ _ _ (or_intror f)).
    + injection eqseed1 as <- <-.
      constructor; [split; [exact eq_refl|split; [|exact eq_refl]]|].
    ...
    + refine (Forall2_impl _ _ IH2); clear - eqnsm.
      ...
      intros ->.
      rewrite H in eqnsm; discriminate eqnsm.
    + intros k vn vt [H|H]; injection eqseed1 as <- <-; [|exact (IH3 ((vn, vt) :: k) vn vt H)].
      exact (ident_diff seed ((""%string, TInt) :: k) ltac:(discriminate) H).
  }
  refine (match _ : { sm & { seed & Result.t _ { n_locals |
                      _ /\ Forall2 _ n_locals _ /\ freshness _ n_locals}}} with
    | existT _ sm (existT _ seed (Result.Ok (exist _ n_locals (conj Hll H0)))) =>
        existT _ sm (existT _ seed (Result.Ok (
          existT _ [] (exist _ n_locals
            (conj (Forall_nil _) (conj Hll (conj (Forall_nil _) (conj (Forall2_nil _) H0))))))))
    | existT _ sm (existT _ seed (Result.Err e)) => existT _ sm (existT _ seed (Result.Err e)) end).
  induction loc as [ | [n ty] vars (sm & seed & IH) ].
  2:{
    destruct (StringMap.find n sm) as [ [ i dl ] | ] eqn:eqnsm.
    1: exists sm, seed;
       exact (Result.Err ((l, Result.MultipleDeclaration n i Result.DeclLocal dl) :: match IH with Result.Ok _ => [] | Result.Err es => es end)).
    destruct (next_ident seed n ty) as [seed2 newb] eqn:eqseed1.
    exists (StringMap.add n (seed, Result.DeclLocal) sm), seed2;
      refine (Result.bind IH _); clear IH;
      intros (IHloc & IHll & IH1 & IH4);
      left; exists (newb :: IHloc).
    repeat match goal with |- _ /\ _ => split end; try assumption.
    + constructor; [intros [bn' bi' bty'] [=->] [[=<-]|H]; [exact eq_refl|exfalso; refine (IH4 [] _ _ _)]|].
      1: injection eqseed1 as <- <-; exact (in_map _ _ _ H).
      assert (Hseed : ~ In (binder_id newb) (map binder_id IHloc))
        by (injection eqseed1 as <- <-; unfold freshness in IH4; exact (fun f => IH4 [] _ _ f)).
      clear - IHll Hseed.
      refine (proj2 (Forall_forall _ _) _).
      intros [vn v tv] Hv [bn' bi' bt'] [=->] [[=f]|H]; [contradict Hseed; subst; exact (in_map _ _ _ Hv)|].
      refine (proj1 (Forall_forall _ _) IHll _ Hv _ _ H); exact eq_refl.
    + injection eqseed1 as <- <-.
      constructor; [split; [exact eq_refl|split; [|exact eq_refl]]|].
      1: exact (StringMap.find_1 (StringMap.add_1 _ _ eq_refl)).
      refine (Forall2_impl _ _ IH1); clear - eqnsm.
  ...
  exists (StringMap.empty _), O; left; exists []; split; [exact (Forall_nil _)|split; [exact (Forall2_nil _)|]].
  intros aux vn vt [].
Defined.
...
Lemma check_Henv {n_in n_out n_loc} :
  Forall (fun v => forall b', binder_id b' = binder_id v -> In b' n_in -> b' = v) n_in ->
  Forall (fun v => forall b', binder_id b' = binder_id v -> In b' n_out -> b' = v) n_out ->
  Forall (fun v => forall b', binder_id b' = binder_id v -> In b' n_loc -> b' = v) n_loc ->
  Forall (fun v => ~ In (binder_id v) (map binder_id n_out)) n_in ->
  Forall (fun v => ~ In (binder_id v) (map binder_id n_loc)) n_in ->
  Forall (fun v => ~ In (binder_id v) (map binder_id n_loc)) n_out ->
  forall b,
    Dict.maps_to (binder_id b) (binder_name b, binder_ty b)
      (fold_left (fun acc b => Dict.add (binder_id b) (binder_name b, binder_ty b) acc)
       (n_in ++ n_out ++ n_loc)
       (Dict.empty _)) <->
    In b (n_in ++ n_out ++ n_loc)%list.
Proof using.
  rename n_in into nin, n_out into nout, n_loc into nloc.
  intros innotdup' outnotdup' locnotdup' innotout' innotloc' outnotloc' b.
  match goal with |- Dict.maps_to _ _ (fold_left ?f ?l ?d) <-> _ =>
    rewrite <-(fold_left_rev_right (fun x y => f y x) l d) end.
  rewrite (in_rev (nin ++ nout ++ nloc) b),
          !rev_app_distr, <-!app_assoc.
  apply Forall_rev in innotdup', innotloc', innotout', outnotloc', outnotdup', locnotdup'.
  specialize (Forall_impl _ (fun v HP b Hb Hin => HP b Hb (proj2 (in_rev _ _) Hin)) innotdup') as innotdup.
  ...
  specialize (Forall_impl _ (fun v HP b Hb Hin => HP b Hb (proj2 (in_rev _ _) Hin)) outnotdup') as outnotdup.
  specialize (Forall_impl _ (fun v Hnin Hin => Hnin (proj2 (in_rev _ _) (eq_ind _ _ Hin _ (map_rev _ _)))) outnotloc') as outnotloc.
  specialize (Forall_impl _ (fun v HP b Hb Hin => HP b Hb (proj2 (in_rev _ _) Hin)) locnotdup') as locnotdup.
  ...
  - intros H.
    ...
    induction l1 as [|b' l1 IH]; swap 1 2.
    1: destruct (ident_dec (binder_id b) (binder_id b')) as [eqi|Hne]; [left; f_equal; cbn in H; rewrite <-eqi in H; clear - eqi H|].
    1:  unfold Dict.maps_to in H; rewrite Dict.maps_to_last_added in H.
    1:  injection H as H.
    1:  destruct b as [? ? t], b' as [? ? t']; cbn in *; congruence.
    ...
    1:  intros b0 H0 b1 Hb Hin; exact (H0 _ Hb (or_intror Hin)).
    1: exact (Dict.maps_to_not_last_added _ _ _ _ _ H Hne).
    clear locnotdup.
    induction l2 as [|b' l1 IH]; swap 1 2.
    1: destruct (ident_dec (binder_id b) (binder_id b')) as [eqi|Hne]; [left; f_equal; cbn in H; rewrite <-eqi in H; clear - eqi H|].
    1:  unfold Dict.maps_to in H; rewrite Dict.maps_to_last_added in H.
    1:  injection H as H.
    1:  destruct b as [? ? t], b' as [? ? t']; cbn in *; congruence.
    ...
    1: exact (Dict.maps_to_not_last_added _ _ _ _ _ H Hne).
    clear outnotdup.
    induction l3 as [|b' l1 IH]; swap 1 2.
    1: destruct (ident_dec (binder_id b) (binder_id b')) as [eqi|Hne]; [left; f_equal; cbn in H; rewrite <-eqi in H; clear - eqi H|].
    1:  unfold Dict.maps_to in H; rewrite Dict.maps_to_last_added in H.
    1:  injection H as H.
    1:  destruct b as [? ? t], b' as [? ? t']; cbn in *; congruence.
  ...
  - rewrite !in_app_iff.
    ...
    2: specialize (proj1 (Forall_forall _ _) outnotloc _ H : ~ In _ _) as notinl1.
    2: rename outnotdup into Hnotdup, l2 into l; clear innotdup locnotdup.
    3: specialize (proj1 (Forall_forall _ _) innotout _ H : ~ In _ _) as notinl2.
    3: specialize (proj1 (Forall_forall _ _) innotloc _ H : ~ In _ _) as notinl1.
    ...
    1-3: induction l as [|[nn nl tl] l IH]; [contradiction H|].
    1-3: destruct H as [[=<-]|H]; [exact (Dict.maps_to_last_added _ _ _)|].
    1-3: destruct (PeanoNat.Nat.eq_dec (binder_id b) nl) as [<-|Hne]; [clear IH|].
    1,3,5: assert (tmp := fun h => proj1 (Forall_forall _ _) Hnotdup _ (or_intror H) _ h (or_introl eq_refl)).
    1-3: rewrite (tmp eq_refl); exact (Dict.maps_to_last_added _ _ _).
    1-3: refine (Dict.maps_to_add _ _ _ _ _ (IH _ H) Hne).
    1-3: refine (Forall_impl _ _ (Forall_inv_tail Hnotdup)).
    1-3: clear; intros x H t H1 Hin; exact (H _ H1 (or_intror Hin)).
Defined.
...
Definition check_node_prop (entry: Source.node): Result.t type Target.node :=
  ...
  do '(existT _ n_in (existT _ n_out (exist _ n_loc
        (conj Hii (conj Hoo (conj Hll (conj Hio (conj Hil (conj Hol (conj Hnin (conj Hnout (conj Hnloc Hseed)))))))))))) <- tmp;
  ...
  let temp := {|
    ...
    env := fold_left (fun acc b => Dict.add (binder_id b) (binder_name b, binder_ty b) acc) (n_in ++ n_out ++ n_loc) (Dict.empty _);
    Henv := Henv;
  |} in
